...
func TestEndToEndScan(t *testing.T) {
  ...
  "packages": {
	...
	for _, r := range records {
		sf := filepath.ToSlash(r.SourceFile)
		switch {
		case r.Ecosystem == "npm" && r.SourceType == "npm-lockfile" && strings.Contains(sf, "/proj/"):
			lockFromProj = true
		case r.Ecosystem == "npm" && r.SourceType == "npm-lockfile" && strings.Contains(sf, "/dup/"):
			lockFromDup = true
